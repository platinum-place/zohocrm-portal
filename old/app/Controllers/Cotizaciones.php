...
class Cotizaciones extends BaseController
{
    public function index(): string
    {
        ...
        if ($this->request->getPost()) {
            ...
            switch ($cotizacion->plan) {
                case "Seguro Incendio Leasing":
                    ...
                    $cotizacion->tipo_equipo = $this->request->getPost("tipo_equipo");
                ...
                case "Vida":
                    ...
                    $cotizacion->garante = $this->request->getPost("garante");
                    $cotizacion->tipo_pago = $this->request->getPost("tipo_pago");
                ...
                default:
                    ...
                    $cotizacion->salvamento = $this->request->getPost("salvamento");
        ...
        foreach ($marcas as $marca) {
            $marcas_sort[]=[
                'id'=>$marca->getEntityId(),
                'name'=>$marca->getFieldValue('Name'),
            ];
        }
        ...
        do {
            ...
            if (!empty($modelos)) {
                ...
                foreach ($modelos_sort as $modelo_sort) {
                    echo '<option value="' . $modelo_sort['id'] . "," . $modelo_sort['tipo'] . '">' . strtoupper($modelo_sort['name']) . " (" . $modelo_sort['tipo'] . ")" . '</option>';
                }
            } else {
    ...
    public function completar(): RedirectResponse
    {
        ...
        $registro = [
            ...
            "Salvamento" => ($this->request->getPost("salvamento")) ? true : false,
            "Garante" => ($this->request->getPost("garante")) ? true : false,
            "Tipo_de_pago" => $this->request->getPost("tipo_pago"),
        ];
    ...
    public function condicionado($id)
    {
        ...
        foreach ($attachments as $attchmentIns) {
            ...
            return $this->response->download($file, null)->setFileName("Condicionado.pdf");
            /*
    ...
    {
        $libreria = new Zoho();
        $libreria->update("Quotes", $id, ["Pendiente" => true]);
        return "Emisión marcada como pendiente";
    }
}
