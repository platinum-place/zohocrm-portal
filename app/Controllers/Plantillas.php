...
{
    public function tua_activos($id)
    {
        $libreria = new Zoho;
        //datos de la tua
        ...
        $criterio = "((Trato:equals:$id) and (Estado:equals:1))";
        $vehiculos = $libreria->searchRecordsByCriteria("Bienes", $criterio);
        return view('plantillas/tua_activo', [
            "tua" => $tua,
            "cliente" => $cliente,
            "corredor" => $corredor,
            "vehiculos" => $vehiculos,
	...
	public function tua_inactivos($id)
    {
        $libreria = new Zoho;
        //datos de la tua
        $tua = $libreria->getRecord("Deals", $id);
        ...
        $corredor = $libreria->getRecord("Accounts", $tua->getFieldValue("Account_Name")->getEntityId());
        //datos de los vehiculos
        $criterio = "((Trato:equals:$id) and (Estado:equals:2))";
        $vehiculos = $libreria->searchRecordsByCriteria("Bienes", $criterio);
        return view('plantillas/tua_inactivo', [
            "tua" => $tua,
            "cliente" => $cliente,
            "corredor" => $corredor,
            "vehiculos" => $vehiculos,
...
// iniciar las librerias de la api para generar excel
    ...
    public function caso($id)
    {
        ...
        $section->addTitle('Resumen '.$caso->getFieldValue("TUA"), 1);
        ...
        foreach ((array)$fotos as $foto) {
			...
			if(in_array(exif_imagetype($imagen) , array(IMAGETYPE_GIF , IMAGETYPE_JPEG ,IMAGETYPE_PNG , IMAGETYPE_BMP)))
			{
				$nombre = uniqid() . '.png';
...
public function accidente($id)
    {
        ...
        foreach ((array)$fotos as $foto) {
            $imagen = $libreria->downloadAttachment("Cases", $caso->getEntityId(), $foto->getId(),  $ruta_servidor);
            if(file_exists($imagen) and in_array(exif_imagetype($imagen) , array(IMAGETYPE_JPEG ,IMAGETYPE_PNG )))
			{
				$nombre = uniqid() . '.png';
	            $ruta_final = $ruta_servidor . $nombre;
	            rename($imagen,   $ruta_final);
